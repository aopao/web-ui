...
layui.define('layer', function(exports){
  ...
  var present = function () {
      var item = $(this)
          ...
          ,vers, verType, value, select;
      // 是否为下拉框
      if(item.parent().hasClass('layui-select-title')) {
          select = item.closest('.layui-form-select').prev('select');
          if(select.attr('lay-verify')) {
              vers = select.attr('lay-verify').split('|');
              verType = select.attr('lay-verType'); //提示方式
              value = select.val();
          } else {
              return false;
          }
      } else {
          vers = item.attr('lay-verify').split('|');
          verType = item.attr('lay-verType'); //提示方式
          value = item.val();
      }
  ...
  $(ELEM).on('blur', '*[lay-verify]', present);
  $(ELEM).on('blur', '.layui-select-title .layui-input', present);
